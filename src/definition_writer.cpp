...
DefinitionWriter::DefinitionWriter(OTF2_Archive *archive,
...
void DefinitionWriter::writeClockProperties() {
  ...
  OTF2_GlobalDefWriter_WriteClockProperties(
      m_globalDefWriter, clockProps[0].s_timerResolution,
      clockProps[0].s_globalOffset, total_length_itr->s_traceLength);
}
...
void DefinitionWriter::writeString() {
  ...
  for (auto string_itr = start_itr; string_itr != end_itr; string_itr++) {
    ...
    OTF2_GlobalDefWriter_WriteString(m_globalDefWriter, id,
                                     string_itr->c_str());
  }
}
...
void DefinitionWriter::writeParadigm() {
  ...
  for (auto paradigm_itr = start_itr; paradigm_itr != end_itr; paradigm_itr++) {

    OTF2_GlobalDefWriter_WriteParadigm(
        m_globalDefWriter, paradigm_itr->s_paradigm, paradigm_itr->s_name,
...
void DefinitionWriter::writeParadigmProperty() {
  ...
  for (auto paradigm_property_itr = start_itr; paradigm_property_itr != end_itr;
       paradigm_property_itr++) {

    OTF2_GlobalDefWriter_WriteParadigmProperty(
        m_globalDefWriter, paradigm_property_itr->s_paradigm,
        paradigm_property_itr->s_property, paradigm_property_itr->s_type,
...
void DefinitionWriter::writeIoParadigm() {
  ...
  for (auto io_paradigm_itr = start_itr; io_paradigm_itr != end_itr;
    ...
    OTF2_GlobalDefWriter_WriteIoParadigm(
        m_globalDefWriter, id, io_paradigm_itr->s_identification,
        io_paradigm_itr->s_name, io_paradigm_itr->s_ioParadigmClass,
        io_paradigm_itr->s_ioParadigmFlags,
        io_paradigm_itr->s_ioProperties.size(), io_props, types, values);
  }
}
...
void DefinitionWriter::writeSystemTreeNode() {
  ...
  for (auto sys_tree_node_itr = start_itr; sys_tree_node_itr != end_itr;
    ...
    OTF2_GlobalDefWriter_WriteSystemTreeNode(
        m_globalDefWriter, id, sys_tree_node_itr->s_name,
        sys_tree_node_itr->s_className, sys_tree_node_itr->s_parent);
  }
}
...
void DefinitionWriter::writeSystemTreeNodeProperty() {
  ...
  for (auto sys_tree_node_property_itr = start_itr;
       sys_tree_node_property_itr != end_itr; sys_tree_node_property_itr++) {

    OTF2_GlobalDefWriter_WriteSystemTreeNodeProperty(
        m_globalDefWriter, sys_tree_node_property_itr->s_systemTreeNode,
        sys_tree_node_property_itr->s_name, sys_tree_node_property_itr->s_type,
...
void DefinitionWriter::writeSystemTreeNodeDomain() {
  ...
  for (auto sys_tree_node_domain_itr = start_itr;
       sys_tree_node_domain_itr != end_itr; sys_tree_node_domain_itr++) {

    OTF2_GlobalDefWriter_WriteSystemTreeNodeDomain(
        m_globalDefWriter, sys_tree_node_domain_itr->s_systemTreeNode,
...
void DefinitionWriter::writeRegion() {
  ...
  for (auto region_itr = start_itr; region_itr != end_itr; region_itr++) {
    ...
    OTF2_GlobalDefWriter_WriteRegion(
        m_globalDefWriter, id, region_itr->s_name, region_itr->s_canonicalName,
        region_itr->s_description, region_itr->s_regionRole,
        region_itr->s_paradigm, region_itr->s_regionFlags,
        region_itr->s_sourceFile, region_itr->s_beginLineNumber,
...
void DefinitionWriter::writeLocationGroup() {
  ...
  for (auto location_group_itr = start_itr; location_group_itr != end_itr;
    ...
    OTF2_GlobalDefWriter_WriteLocationGroup(
        m_globalDefWriter, id, location_group_itr->s_name,
...
void DefinitionWriter::writeLocation() {
  ...
  for (auto location_itr = start_itr; location_itr != end_itr; location_itr++) {
    ...
    OTF2_GlobalDefWriter_WriteLocation(
        m_globalDefWriter, id, location_itr->s_name,
        location_itr->s_locationType, location_itr->s_numberOfEvents,
...
void DefinitionWriter::writeGroup() {
  ...
  for (auto group_itr = start_itr; group_itr != end_itr; group_itr++) {
    ...
    OTF2_GlobalDefWriter_WriteGroup(
        m_globalDefWriter, id, group_itr->s_name, group_itr->s_groupType,
        group_itr->s_paradigm, group_itr->s_groupFlags,
        group_itr->s_groupMembers.size(), p);
  }
}
...
void DefinitionWriter::writeComm() {
  ...
  for (auto comm_itr = start_itr; comm_itr != end_itr; comm_itr++) {
    ...
    OTF2_GlobalDefWriter_WriteComm(m_globalDefWriter, id, comm_itr->s_name,
                                   comm_itr->s_group, comm_itr->s_parent);
  }
}
...
void DefinitionWriter::writeParameter() {
  ...
  for (auto parameter_itr = start_itr; parameter_itr != end_itr;
    ...
    OTF2_GlobalDefWriter_WriteParameter(m_globalDefWriter, id,
...
void DefinitionWriter::writeAttribute() {
  ...
  for (auto attribute_itr = start_itr; attribute_itr != end_itr;
    ...
    OTF2_GlobalDefWriter_WriteAttribute(
        m_globalDefWriter, id, attribute_itr->s_name,
        attribute_itr->s_description, attribute_itr->s_type);
  }
}
...
void DefinitionWriter::writeIoFileDefinition() {
  ...
  for (auto io_itr = start_itr; io_itr != end_itr; io_itr++) {
    ...
    if (io_itr->s_ioType == "Directory") {
      OTF2_GlobalDefWriter_WriteIoDirectory(m_globalDefWriter, id,
                                            io_itr->s_name, io_itr->s_scope);
    } else {
      OTF2_GlobalDefWriter_WriteIoRegularFile(m_globalDefWriter, id,
                                              io_itr->s_name, io_itr->s_scope);
    }
    ...
    if (io_itr->s_ioProperty.size() > 0) {
      ...
      for (set_itr = io_itr->s_ioProperty.begin();
           set_itr != io_itr->s_ioProperty.end(); ++set_itr) {

        value.uint32 = get<2>(*set_itr);

        OTF2_GlobalDefWriter_WriteIoFileProperty(
            m_globalDefWriter, id, get<0>(*set_itr), get<1>(*set_itr), value);
      }
    }
  }
}
...
void DefinitionWriter::writeIoHandle() {
  ...
  for (auto io_tuple = start_itr; io_tuple != end_itr; io_tuple++) {
    ...
    OTF2_GlobalDefWriter_WriteIoHandle(m_globalDefWriter, id, io_tuple->s_name,
                                       io_tuple->s_file, io_tuple->s_ioParadigm,
                                       io_tuple->s_IoHandleFlags,
                                       io_tuple->s_comm, io_tuple->s_parent);
  }
}
...
void DefinitionWriter::writeIoPreCreatedHandle() {
  ...
  for (auto io_tuple = start_itr; io_tuple != end_itr; io_tuple++) {
    OTF2_GlobalDefWriter_WriteIoPreCreatedHandleState(
        m_globalDefWriter, io_tuple->s_IoHandle, io_tuple->s_mode,
