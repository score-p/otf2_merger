...
int main(int argc, char **argv) {
  ...
  OTF2_Archive* archive =
    OTF2_Archive_Open( args.m_archive_path.c_str(),
                       args.m_archive_name.c_str(),
                       OTF2_FILEMODE_WRITE,
                       CHUNK_SIZE_EVENTS,
                       CHUNK_SIZE_DEFS,
                       file_substrate,
 ...
 for ( size_t i{}; i < args.m_trace_files.size(); i++ )
 {
    ...
    DefinitionReader defReader(
      traceDefs, maps, reader, args.m_trace_files[ i ], args.m_comm_group_pattern + "_" + to_string(i));
